 ...
 */
int two_sides_spread(uint16_t r, uint16_t g, uint16_t b, uint8_t speed, uint8_t tempo, uint8_t *firstIn)
{
    static uint8_t start_pos = TOTAL_LEDS/2;
...
// 从白光过渡到蓝光，暂时没有实现
int white2blue(uint8_t *firstIn)
{
    ...
    for(i = 0;i < 10;i++)
    {
    	...
    	update_according_array(leds,0x00,0x00,0x3f);
    }
    return 1;
}
 ...
 *      @firstIn: 控制效果重新刷新的
 */
void light_welcome(uint8_t *firstIn)
{
    ...
    {
        case 0:
            if(two_sides_spread(0x3f, 0x3f, 0x3f, spreadSpeed, spreadTempo, &firstInTwoSidesSpread) == 1) {
                cur_period = 1;
            }
            break;

        case 1:
            if(white2blue(&firstInWhite2Blue) == 1) {
                cur_period = 2;
            }
            break;
...
void light_goodbye(uint8_t *firstIn)
{
    ...
    switch(cur_period)
    {
        ...
        case 1:
            if(shrink2center(0, 0, 0x7f, shrinkSpeed, shrinkTempo, shrinkCond, &firstInShrink) == 1) {
                cur_period = 2;
            }
            break;
